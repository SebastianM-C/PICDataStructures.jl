...
@propagate_inbounds function Base.getindex(f::T, I::Vararg{UnitRange}) where {T<:AbstractPICDataStructure}
    data = getindex(scalarness(T), f, I)
    old_grid = getdomain(f)
    G = typeof(old_grid)
    axis_names = propertynames(old_grid)
    nt = (;(axis_names .=> map(((g,i),)->g[i], zip(old_grid, I)))...)
    grid = parameterless_type(G)(nt)
